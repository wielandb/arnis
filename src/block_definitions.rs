...
pub const BUSH: Block = Block::new(212);
pub const LEAF_LITTER: Block = Block::new(213);
pub const SHORT_DRY_GRASS: Block = Block::new(214);
pub const POTTED_DEAD_BUSH: Block = Block::new(215);

/// All known blocks for name lookup.
pub const ALL_BLOCKS: &[Block] = &[
    ...
    FERN,
    COBWEB,
    CHISELLED_BOOKSHELF_NORTH,
    CHISELLED_BOOKSHELF_EAST,
    CHISELLED_BOOKSHELF_SOUTH,
    CHISELLED_BOOKSHELF_WEST,
    CHIPPED_ANVIL,
    DAMAGED_ANVIL,
    LARGE_FERN_LOWER,
    LARGE_FERN_UPPER,
...
static BLOCKS_BY_NAME: Lazy<HashMap<&'static str, Block>> = Lazy::new(|| {
    let mut map = HashMap::new();
    for block in ALL_BLOCKS {
        map.entry(block.name()).or_insert(*block);
    }
    map
});

pub fn block_from_java_name(name: &str) -> Option<Block> {
    let normalized = name.strip_prefix("minecraft:").unwrap_or(name);
    BLOCKS_BY_NAME.get(normalized).copied()
}
